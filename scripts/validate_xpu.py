...
def validate_environment():
    try:
        ...
        if not hasattr(torch, 'xpu'):
            print('FAIL: PyTorch installed, but "torch.xpu" is missing.')
            return 1
        
        # Check 2: Is the hardware visible?
        if not torch.xpu.is_available():
            print('FAIL: torch.xpu exists, but device is not available (Driver/Kernel issue).')
            return 1
...
def check_hardware():
    try:
        if torch.xpu.is_available():
            ...
            print('FAILURE: XPU still not accessible after repair.')
            return 1
    except Exception as e:
